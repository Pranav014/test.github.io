...
def main(url):
    ...
    #print(' Time to get features: ',time_taken1)
    #start = time.time()
    result.loc[0] = results
    result = result.drop(['url','label'],axis=1).values
    print(saved_model.predict(result))
    ...
    #print('Time to predict: ',time_taken2)
    #start = time.time()
    ans=saved_model.predict(result)
    predict_status=int(ans[0])
    ...
    #print('Time to get certi status: ',time_taken3)
    #total = time_taken1 + time_taken2 + time_taken3
